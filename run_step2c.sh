...
SIM_FILE="${OUTPUT_DIR}/simulation_data/qam8r1t2simser.dat"
> "${SIM_FILE}"

print_msg "${BLUE}========================================${NC}"
print_msg "${BLUE}Step 2 Case (c): 8-QAM, Nr=1, Nt=2${NC}"
...
if [ $? -ne 0 ]; then
    ...
    if [ -f "mimodata/qam8r1t2thser.dat" ]; then
        cp mimodata/qam8r1t2thser.dat "${OUTPUT_DIR}/simulation_data/"
        print_msg "${GREEN}✓ Union bound generated${NC}"
    fi
    
    if [ -f "mimodata/qam8r1t2chser.dat" ]; then
        cp mimodata/qam8r1t2chser.dat "${OUTPUT_DIR}/simulation_data/"
...
GNUPLOT_SCRIPT="${OUTPUT_DIR}/plots/plot_case_c.gp"
...
if [ -f "${OUTPUT_DIR}/plots/case_c_8qam_nr1_nt2.png" ]; then
    print_msg "${GREEN}✓ PNG plot generated${NC}"
fi

if [ -f "${OUTPUT_DIR}/plots/case_c_8qam_nr1_nt2.eps" ]; then
    print_msg "${GREEN}✓ EPS plot generated${NC}"
fi

print_msg "\n${GREEN}========================================${NC}"
print_msg "${GREEN}Case (c) complete!${NC}"
