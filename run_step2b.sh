...
SIM_FILE="${OUTPUT_DIR}/simulation_data/qam8r2t1simser.dat"
> "${SIM_FILE}"

print_msg "${BLUE}========================================${NC}"
print_msg "${BLUE}Step 2 Case (b): 8-QAM, Nr=2, Nt=1${NC}"
...
if [ $? -ne 0 ]; then
    ...
    if [ -f "mimodata/qam8r2t1thser.dat" ]; then
        cp mimodata/qam8r2t1thser.dat "${OUTPUT_DIR}/simulation_data/"
        print_msg "${GREEN}✓ Union bound generated${NC}"
    fi
    
    if [ -f "mimodata/qam8r2t1chser.dat" ]; then
        cp mimodata/qam8r2t1chser.dat "${OUTPUT_DIR}/simulation_data/"
...
GNUPLOT_SCRIPT="${OUTPUT_DIR}/plots/plot_case_b.gp"
...
if [ -f "${OUTPUT_DIR}/plots/case_b_8qam_nr2_nt1.png" ]; then
    print_msg "${GREEN}✓ PNG plot generated${NC}"
fi

if [ -f "${OUTPUT_DIR}/plots/case_b_8qam_nr2_nt1.eps" ]; then
    print_msg "${GREEN}✓ EPS plot generated${NC}"
fi

print_msg "\n${GREEN}========================================${NC}"
print_msg "${GREEN}Case (b) complete!${NC}"
